...
class MM_SOW_Admin_Ajax {
    ...
    public function init_hooks() {
        ...
        add_action('wp_ajax_mm_sow_admin_ajax', array($this, 'mm_sow_admin_ajax'));
    ...
    public function mm_sow_check_nonce() {

        // retrieve nonce
        $nonce = (isset($_POST['nonce'])) ? $_POST['nonce'] : $_GET['nonce'];

        // nonce action for the grid
        $action = 'mm_sow_admin_nonce';

        // check ajax nounce
        if (!wp_verify_nonce($nonce, $action)) {
            // build response
            $response = $this->ajax_response(false, __('Sorry, an error occurred. Please refresh the page.', 'mm_sow'));
    ...
    public function mm_sow_admin_ajax() {

        // check the nonce
        $this->mm_sow_check_nonce();
        ...
        switch ($func) {
            case 'mm_sow_save_settings':
                $response = $this->save_settings_callback();
                break;
            case 'mm_sow_reset_settings':
                $response = $this->save_settings_callback();
                break;
            default:
                $response = ajax_response(false, __('Sorry, an unknown error occurred...', 'mm_sow'), null);
                break;
        }
    ...
    public function save_settings_callback() {
        ...
        mm_sow_update_options($setting_data);
    ...
    public function localize_strings() {
        ...
        $this->ajax_msg = array(
            ...
            'box_messages' => array(
                ...
                'mm_sow_save_settings' => array(
                    'before' => __('Saving plugin settings', 'mm_sow'),
                    'success' => __('Plugin settings Saved', 'mm_sow'),
                    'error' => __('Sorry, an error occurs while saving settings...', 'mm_sow')
                ),
                'mm_sow_reset_settings' => array(
                    'before' => __('Resetting plugin settings', 'mm_sow'),
                    'success' => __('Plugin settings resetted', 'mm_sow'),
                    'error' => __('Sorry, an error occurred while resetting settings', 'mm_sow')
                ),
            )
        );
    ...
    public function admin_nonce() {
        ...
        return array(
            ...
            'nonce' => wp_create_nonce('mm_sow_admin_nonce')
        );
    ...
    public function enqueue_admin_scripts() {
        ...
        if (strpos($screen->id, $this->plugin_slug) !== false) {
            ...
            wp_register_script('mm_sow-admin-ajax-scripts', MM_SOW_PLUGIN_URL . 'admin/assets/js/mm_sow-admin-ajax' . $suffix . '.js', array(), MM_SOW_VERSION, true);
            wp_enqueue_script('mm_sow-admin-ajax-scripts');

            wp_localize_script('mm_sow-admin-ajax-scripts', 'mm_sow_admin_global_var', $strings);
