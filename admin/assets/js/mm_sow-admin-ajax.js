...
/*global mm_sow_admin_global_var*/
...
// ==================================================================
    ...
    var xhr,
        msg_strings = mm_sow_admin_global_var.box_messages,
        msg_icons = mm_sow_admin_global_var.box_icons;
...
// ===================================================================
    ...
    var infobox = '#mm_sow-infobox',
        infobox_msg = '.mm_sow-infobox-msg',
        infobox_loading = 'mm_sow-infobox-loading';

    $(document).on('click', infobox, function (e) {
        if ($(infobox).is(':visible') && ($(e.target).is('.mm_sow-info-inner') || $(e.target).is('.mm_sow-close-infobox'))) {
    ...
    function show_message_error(ajax_data, response) {
        $(infobox_msg).html(mm_sow_admin_global_var.box_icons.error + response.message);
        setTimeout(function () {
            $(infobox).removeClass(infobox_loading);
        }, 1500);
    }
...
// ==================================================================
    ...
    $(document).on('click', '#mm_sow_settings_save, #mm_sow_settings_reset', function () {

        var func = $(this).data('action'),
            reset = (func === 'mm_sow_reset_settings') ? true : null,
            setting_data = get_settings_data(reset);

        Ajax_Helper({
            nonce: mm_sow_admin_global_var.nonce,
            action: 'mm_sow_admin_ajax',
            func: func,
            reset: reset,
    ...
    function show_save_settings_success(ajax_data, response, msg) {
        ...
        setTimeout(function () {
            $(infobox).removeClass(infobox_loading);
            if (response.content) {
                $('.mm_sow-settings').html($(response.content).html());
                MM_SOW_JS.init();
            }
        }, 800);
    }
